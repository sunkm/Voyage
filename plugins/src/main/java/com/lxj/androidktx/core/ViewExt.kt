...
fun View.animateGone(duration: Long = 250, fade: Boolean = true, move: Boolean = true) {
    TransitionManager.beginDelayedTransition(parent as ViewGroup,
        TransitionSet().setDuration(duration).apply {
                if (move) addTransition(MaterialSharedAxis(MaterialSharedAxis.Y, true))
                if (fade) addTransition(Fade())
            }.addTransition(ChangeScroll()).addTransition(ChangeBounds()))
    visibility = View.GONE
}
...
fun View.animateInvisible(duration: Long = 250) {
    TransitionManager.beginDelayedTransition(parent as ViewGroup,
        TransitionSet().setDuration(duration).addTransition(Fade()).addTransition(ChangeBounds()))
    visibility = View.INVISIBLE
}
...
fun View.toBitmap(): Bitmap {
    ...
    return when (this) {
        is RecyclerView -> {
            ...
            this.measure(View.MeasureSpec.makeMeasureSpec(measuredWidth, View.MeasureSpec.EXACTLY),
        ...
        is ScrollView, is HorizontalScrollView, is NestedScrollView -> {
            ...
            val screenshot = Bitmap.createBitmap(child.measuredWidth,
