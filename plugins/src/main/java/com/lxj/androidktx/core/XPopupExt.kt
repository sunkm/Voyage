 ...
 * 直接监听state状态
 */
fun BasePopupView.observeState(
    ...
    liveData.state.observe(owner, Observer<StateLiveData.State> {
        bindState(
            liveData, onLoading = onLoading, onSuccess = onSuccess,
...
//view model
fun <T : ViewModel> BasePopupView.getVM(clazz: Class<T>) =
    ViewModelProvider(context as FragmentActivity)[clazz]
 ...
 * ```
 */
fun <T : ViewModel> BasePopupView.getSavedStateVM(clazz: Class<T>) = ViewModelProvider(
    context as FragmentActivity, SavedStateViewModelFactory(
