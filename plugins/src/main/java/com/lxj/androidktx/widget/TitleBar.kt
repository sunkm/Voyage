...
class TitleBar @JvmOverloads constructor(
    ...
    private var rightImage3Padding = 12
        set(value) {
            field = value
            applyAttr()
            applySelf()
        }
    private var bottomLine = Color.TRANSPARENT
        set(value) {
            field = value
            applyAttr()
            applySelf()
    ...
    init {
        ...
        leftTextDrawableSize = ta.getDimensionPixelSize(
            R.styleable.TitleBar_leftTextDrawableSize,
            leftTextDrawableSize
        )
        ...
        leftImagePadding =
            ta.getDimensionPixelSize(R.styleable.TitleBar_leftImagePadding, leftImagePadding.dp)
        ...
        rightTextSize =
            ta.getDimensionPixelSize(R.styleable.TitleBar_rightTextSize, rightTextSize.sp)

        rightImage = ta.getDrawable(R.styleable.TitleBar_rightImageSrc)
        rightImagePadding =
            ta.getDimensionPixelSize(R.styleable.TitleBar_rightImagePadding, rightImagePadding.dp)
        rightImage2 = ta.getDrawable(R.styleable.TitleBar_rightImage2Src)
        rightImage2Padding =
            ta.getDimensionPixelSize(R.styleable.TitleBar_rightImage2Padding, rightImage2Padding.dp)
        rightImage3 = ta.getDrawable(R.styleable.TitleBar_rightImage3Src)
        rightImage3Padding =
            ta.getDimensionPixelSize(R.styleable.TitleBar_rightImage3Padding, rightImage3Padding.dp)
    ...
    override fun dispatchDraw(canvas: Canvas) {
        ...
        if (bottomLine != 0) {
            ...
            canvas.drawRect(
                Rect(0, (measuredHeight) - 1.dp.toInt(), measuredWidth, measuredHeight),
                paint
            )
        }
    }
    ...
    private fun applyLeftText() {
        ...
        if (leftTextDrawable != null) {
            ...
            if (scale > 1) {
                tvLeftText.sizeDrawable(
                    width = leftTextDrawableSize,
                    height = (leftTextDrawableSize / scale).toInt()
                )
            } else {
                tvLeftText.sizeDrawable(
                    width = (leftTextDrawableSize / scale).toInt(),
    ...
    fun setupLeftText(
        text: CharSequence, textColor: Int = leftTextColor, textSize: Int = leftTextSize,
        drawable: Drawable? = leftTextDrawable, drawableSize: Int = leftTextDrawableSize,
    ): TitleBar {
        this.leftText = text
    ...
    fun setupTitle(
        text: CharSequence,
        textColor: Int = titleColor,
        textSize: Int = titleSize,
        alignLeft: Boolean = titleAlignLeft,
    ): TitleBar {
        this.title = text
