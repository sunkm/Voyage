...
class MediaSessionManager(
    ...
    private fun updatePlaybackState() {
        ...
        mediaSession?.setPlaybackState(
            PlaybackStateCompat.Builder().setActions(MEDIA_SESSION_ACTIONS)
                .setState(state, getCurrentPosition(), 1f).build()
        )
    }

    private fun getCurrentPosition(): Long =
        playbackManager.player()?.currentStreamPosition().orDef()
    ...
    fun updateMetaData(songInfo: SongInfo?) {
        ...
        val metaDta = MediaMetadataCompat.Builder()
            ...
            .putString(MediaMetadataCompat.METADATA_KEY_ALBUM_ARTIST, songInfo.artist).putLong(
                MediaMetadataCompat.METADATA_KEY_DURATION,
                playbackManager.player()?.duration().orDef()
            ) //加上这个通知栏会有进度条
