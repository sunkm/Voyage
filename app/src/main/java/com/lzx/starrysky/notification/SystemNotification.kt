...
class SystemNotification constructor(
    ...
    private fun createNotification(): Notification? {
        ...
        notificationBuilder.setStyle(androidx.media.app.NotificationCompat.MediaStyle()
            ...
            .setCancelButtonIntent(mStopIntent).setMediaSession(mediaSession))
            .setDeleteIntent(mStopIntent).setColorized(true).setSmallIcon(smallIcon)
            .setOngoing(true).setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
            .setOnlyAlertOnce(true).setChronometerCountDown(true)
        ...
        if (!config.targetClass.isNullOrEmpty()) {
            ...
            clazz?.let {
                val intent = NotificationUtils.createContentIntent(context,
                    config,
                    songInfo,
    ...
    private fun addActions(notificationBuilder: NotificationCompat.Builder): Int {
        ...
        if (hasPreSong) {
            ...
            val title =
                config.skipPreviousTitle.ifEmpty { context.getString(R.string.label_previous) }
        ...
        if (playbackState == PlaybackStage.PLAYING || playbackState == PlaybackStage.BUFFERING) {
            label = config.labelPlay.ifEmpty { context.getString(R.string.label_pause) }
            icon =
                if (config.pauseDrawableRes != -1) config.pauseDrawableRes else R.drawable.ic_pause_white_24dp
            intent = mPauseIntent
        } else {
            label = config.labelPause.ifEmpty { context.getString(R.string.label_play) }
            icon =
        ...
        if (hasNextSong) {
            ...
            val title = config.skipNextTitle.ifEmpty { context.getString(R.string.label_next) }
