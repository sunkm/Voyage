...
class FocusManager(val context: Context) {
    ...
    private fun shouldAbandonAudioFocus(playbackState: Int): Boolean {
        return playbackState == Playback.STATE_IDLE || focusGain != AUDIOFOCUS_GAIN
    }
    ...
    private fun requestAudioFocusV26(): Int {
        if (audioFocusRequest == null || rebuildAudioFocusRequest) {
            ...
            audioFocusRequest = builder.setAudioAttributes(audioAttributes.audioAttributesV21 as android.media.AudioAttributes)
        ...
        return audioFocusRequest?.let { audioManager?.requestAudioFocus(it) } ?: 0
    }
    ...
    private fun convertAudioAttributesToFocusGain(audioAttributes: AudioAttributes): Int {
        return if (audioAttributes == null) {
            ...
            C.USAGE_UNKNOWN -> AUDIOFOCUS_GAIN
            ...
            C.USAGE_NOTIFICATION_RINGTONE,
            -> AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK
            C.USAGE_ASSISTANT -> if (Util.SDK_INT >= 19) AUDIOFOCUS_GAIN_TRANSIENT_EXCLUSIVE else AUDIOFOCUS_GAIN_TRANSIENT
            C.USAGE_ASSISTANCE_ACCESSIBILITY -> {
                if (audioAttributes.contentType == C.CONTENT_TYPE_SPEECH) AUDIOFOCUS_GAIN_TRANSIENT else AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK
            }
    ...
    private fun setAudioFocusState(audioFocusState: Int) {
        ...
        val volumeMultiplier =
            if (audioFocusState == STATE_LOSS_TRANSIENT_DUCK) VOLUME_DUCK else VOLUME_NORMAL
    ...
    private fun handlePlatformAudioFocusChange(focusChange: Int) {
        when (focusChange) {
            AudioManager.AUDIOFOCUS_GAIN -> {
                ...
                listener?.focusStateChange(FocusInfo(null,
                    audioFocusState,
                    playerCommand,
                    volumeMultiplier))
            }
