...
class ExoPlayback(
    ...
    private fun String.hasMediaSource(): Boolean = runCatching {
    ...
    private fun createExoPlayer() {
        if (player == null) {
            ...
            player =
                SimpleExoPlayer.Builder(context, renderersFactory).setTrackSelector(trackSelector!!)
                    .build()
    ...
    private fun buildCacheDataSource(
        ...
        return cache?.let {
            CacheDataSource.Factory().setCache(it).setUpstreamDataSourceFactory(upstreamFactory)
...
//        player?.removeAnalyticsListener(analyticsListener)
    ...
    override fun focusStateChange(info: FocusInfo) {
        ...
        callback?.onFocusStateChange(FocusInfo(currSongInfo,
