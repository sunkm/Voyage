...
fun Context.getPendingIntent(requestCode: Int, action: String): PendingIntent {
    ...
    return PendingIntent.getBroadcast(this, requestCode, intent, if (Build.VERSION.SDK_INT >= 31) PendingIntent.FLAG_IMMUTABLE else PendingIntent.FLAG_CANCEL_CURRENT)
}
...
fun String.isRTMP(): Boolean {
    return this.lowercase(Locale.getDefault()).startsWith("rtmp://")
}

fun String.isFLAC(): Boolean {
    return this.lowercase(Locale.getDefault()).endsWith(".flac")
}
