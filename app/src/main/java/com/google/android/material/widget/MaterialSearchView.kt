...
import com.manchuan.tools.databinding.MaterialSearchViewBinding
...
class MaterialSearchView @JvmOverloads constructor(
    ...
    init {
        ...
        val left = context.resources.getDimensionPixelSize(R.dimen.dp_8)
        ...
        when {
            a.hasValue(R.styleable.MaterialSearchView_search_navigationIconCompat) -> {
                navigationIconCompat =
                    a.getInt(R.styleable.MaterialSearchView_search_navigationIconCompat,
        ...
        if (a.hasValue(R.styleable.MaterialSearchView_search_navigationElevation)) {
            val navigationElevation =
                a.getDimensionPixelSize(R.styleable.MaterialSearchView_search_navigationElevation,
                    0)
        ...
        if (a.hasValue(R.styleable.MaterialSearchView_search_clearIcon)) {
            ...
            setClearIcon(ContextCompat.getDrawable(context, R.drawable.search_ic_outline_clear_24))
        }
    ...
    fun showKeyboard() {
        if (!isInEditMode) {
            ...
            inputMethodManager.showSoftInput(binding.searchViewEditText,
    ...
    fun hideKeyboard() {
        if (!isInEditMode) {
            val inputMethodManager =
                context.getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
            inputMethodManager.hideSoftInputFromWindow(binding.searchViewEditText.windowToken,
                InputMethodManager.RESULT_UNCHANGED_SHOWN)
        }
    }
