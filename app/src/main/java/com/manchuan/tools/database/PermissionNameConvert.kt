        ...
        for (permission in permissions) {
            when (permission) {
                ...
                Permission.READ_MEDIA_AUDIO -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                        val hint: String = context.getString(R.string.common_permission_audio)
                        if (!permissionNames.contains(hint)) {
                            permissionNames.add(hint)
                        }
                    }
                ...
                Permission.BLUETOOTH_SCAN, Permission.BLUETOOTH_CONNECT, Permission.BLUETOOTH_ADVERTISE -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                        val hint: String =
                            context.getString(R.string.common_permission_wireless_devices)
                        if (!permissionNames.contains(hint)) {
                            permissionNames.add(hint)
                        }
                    }
                ...
                Permission.ACCESS_MEDIA_LOCATION -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                        val hint: String =
                            context.getString(R.string.common_permission_media_location)
                        if (!permissionNames.contains(hint)) {
                            permissionNames.add(hint)
                        }
                    }
                ...
                Permission.MANAGE_EXTERNAL_STORAGE -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                        val hint: String =
                            context.getString(R.string.common_permission_manage_storage)
                        if (!permissionNames.contains(hint)) {
                            permissionNames.add(hint)
                        }
                    }
                ...
                Permission.POST_NOTIFICATIONS -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                        val hint: String =
                            context.getString(R.string.common_permission_post_notifications)
                        if (!permissionNames.contains(hint)) {
                            permissionNames.add(hint)
                        }
                    }
                ...
                Permission.BIND_NOTIFICATION_LISTENER_SERVICE -> {
                    val hint: String =
                        context.getString(R.string.common_permission_notification_listener)
                    if (!permissionNames.contains(hint)) {
                        permissionNames.add(hint)
                    }
                }
                Permission.PACKAGE_USAGE_STATS -> {
                    val hint: String = context.getString(R.string.common_permission_task)
                    if (!permissionNames.contains(hint)) {
                        permissionNames.add(hint)
                    }
                }
                Permission.SCHEDULE_EXACT_ALARM -> {
                    val hint: String = context.getString(R.string.common_permission_alarm)
                    if (!permissionNames.contains(hint)) {
                        permissionNames.add(hint)
                    }
                }
                Permission.ACCESS_NOTIFICATION_POLICY -> {
                    val hint: String = context.getString(R.string.common_permission_not_disturb)
                    if (!permissionNames.contains(hint)) {
                        permissionNames.add(hint)
                    }
                }
                Permission.REQUEST_IGNORE_BATTERY_OPTIMIZATIONS -> {
                    val hint: String = context.getString(R.string.common_permission_ignore_battery)
                    if (!permissionNames.contains(hint)) {
                        permissionNames.add(hint)
                    }
                }
                Permission.BIND_VPN_SERVICE -> {
                    val hint: String = context.getString(R.string.common_permission_vpn)
                    if (!permissionNames.contains(hint)) {
                        permissionNames.add(hint)
                    }
                }
