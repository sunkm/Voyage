...
class TOImageActivity : AppCompatActivity() {
    ...
    private fun video2Image() {
        val dir = File(externalPicturesDirPath + File.separator + "images", randomUUIDString)
        if (!dir.exists()) {
            dir.mkdirs()
        }
        val targetPath = dir.absolutePath
        GlobalScope.launch {
            FFmpegCommand.runCmd(FFmpegUtils.video2Image(path, targetPath,
                ImageFormat.PNG), callback(targetPath))
        }
    }
