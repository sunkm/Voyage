...
import com.manchuan.tools.databinding.ActivityBrightBinding
...
class BrightActivity : AppCompatActivity() {
    ...
    private val binding by lazy {
        ActivityBrightBinding.inflate(layoutInflater)
    }
    ...
    private fun bright(bright: Float) {
        targetPath =
            externalMoviesDirPath.toString() + File.separator + "${SystemClock.elapsedRealtime()}.mp4"
        GlobalScope.launch {
            FFmpegCommand.runCmd(FFmpegUtils.videoBright(path, targetPath, bright = bright),
                callback(targetPath))
        }
    }
