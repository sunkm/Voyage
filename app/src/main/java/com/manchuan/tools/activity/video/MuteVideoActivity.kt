...
class MuteVideoActivity : AppCompatActivity() {
    ...
    private fun reduceAudio() {
        val targetPath = externalMoviesDirPath.toString() + File.separator + "${SystemClock.elapsedRealtime()}.mp4"
        GlobalScope.launch {
            FFmpegCommand.runCmd(FFmpegUtils.extractVideo(path, targetPath),
                callback(targetPath))
        }
    }
