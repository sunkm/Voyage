...
class H264Activity : AppCompatActivity() {
    ...
    private fun yuv2H264() {
        val dir = File(externalMoviesDirPath + File.separator + "H264", randomUUIDString)
        if (!dir.exists()) {
            dir.mkdirs()
        }
        val targetPath = dir.toString() + File.separator + "${System.currentTimeMillis()}.h264"
        GlobalScope.launch {
            FFmpegCommand.runCmd(FFmpegUtils.yuv2H264(path,
                targetPath,
                FFmpegCommand.getMediaInfo(path, MediaAttribute.WIDTH)!!,
                FFmpegCommand.getMediaInfo(path, MediaAttribute.HEIGHT)!!), callback(targetPath))
        }
    }
