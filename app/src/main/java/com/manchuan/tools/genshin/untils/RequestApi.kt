package com.manchuan.tools.genshin.untils
...
import com.manchuan.tools.genshin.information.ResponseCode
import okhttp3.*
    ...
    companion object {
        ...
        private val client by lazy {
            OkHttpClient.Builder().addInterceptor {
                ...
                request.addHeader(
                    "DS",
                    getDS("role_id=${mainUser?.gameUid}&server=${mainUser?.region}")
                )
        ...
        fun dailySign(user: UserBean, block: (JSONObject) -> Unit) {
            ...
            val request = Request.Builder().url(MiHoYoAPI.DAILY_SIGN).post(toRequestBody)
                ...
                .addHeader(
                    "Cookie",
                    "account_id=${user.loginUid};cookie_token=${user.cookieToken};"
                )
                .build()
...
class MyCallBack(val block: (JSONObject) -> Unit) : Callback {
    ...
    override fun onResponse(call: Call, response: Response) {
        runCatching {
            ...
            loge("请求数据", response.body!!.string())
            loge(it.toString())
            block(
                JSONObject(
                    """
