...
class BaiJiaFragment : EngineFragment<FragmentSpeakAudioBinding>(R.layout.fragment_speak_audio) {
    ...
    override fun initView() {
        ...
        receiveEvent<ArrayList<GameModel>>("models_bj") {
            binding.recyclerView.models = it
        }
        receiveEvent<ArrayList<GameModel>>("load_more_models_bj") {
        ...
        binding.page.onRefresh {
            ...
            runCatching {
                Thread {
                    audioList.clear()
                    val document = Jsoup.connect("http://www.tingbook.cc/book/33-$page.html").get()
        ...
        binding.page.onLoadMore {
            runCatching {
                Thread {
                    ...
                    loge(stringBuilder.toString())
                    sendEvent(audioList, "load_more_models_bj")
                    page++
                    this.finish(true)
                }.start()
            }.onFailure {
