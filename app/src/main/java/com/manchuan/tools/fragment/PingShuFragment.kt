...
class PingShuFragment : EngineFragment<FragmentSpeakAudioBinding>(R.layout.fragment_speak_audio) {
    ...
    override fun initView() {
        ...
        receiveEvent<ArrayList<GameModel>>("models_ps") {
            binding.recyclerView.models = it
        }
        receiveEvent<ArrayList<GameModel>>("load_more_models_ps") {
        ...
        binding.page.onRefresh {
            ...
            runCatching {
                Thread {
                    audioList.clear()
                    val document = Jsoup.connect("http://www.tingbook.cc/book/13-$page.html").get()
        ...
        binding.page.onLoadMore {
            runCatching {
                Thread {
                    ...
                    loge(stringBuilder.toString())
                    sendEvent(audioList, "load_more_models_ps")
                    page++
                    this.finish(true)
                }.start()
            }.onFailure {
