...
class LiShiFragment : EngineFragment<FragmentSpeakAudioBinding>(R.layout.fragment_speak_audio) {
    ...
    override fun initView() {
        ...
        receiveEvent<ArrayList<GameModel>>("models_ls") {
            binding.recyclerView.models = it
        }
        receiveEvent<ArrayList<GameModel>>("load_more_models_ls") {
        ...
        binding.page.onRefresh {
            ...
            runCatching {
                Thread {
                    audioList.clear()
                    val document = Jsoup.connect("http://www.tingbook.cc/book/9-$page.html").get()
        ...
        binding.page.onLoadMore {
            runCatching {
                Thread {
                    ...
                    loge(stringBuilder.toString())
                    sendEvent(audioList, "load_more_models_ls")
                    page++
                    this.finish(true)
                }.start()
            }.onFailure {
