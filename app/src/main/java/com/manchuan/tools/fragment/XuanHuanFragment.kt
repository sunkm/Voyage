...
class XuanHuanFragment : EngineFragment<FragmentSpeakAudioBinding>(R.layout.fragment_speak_audio) {
    ...
    override fun initView() {
        ...
        binding.recyclerView.staggered(2, StaggeredGridLayoutManager.VERTICAL).setup {
        ...
        receiveEvent<ArrayList<GameModel>>("models_xh") {
            binding.recyclerView.models = it
        }
        receiveEvent<ArrayList<GameModel>>("load_more_models_xh") {
        ...
        binding.page.onRefresh {
            ...
            runCatching {
                Thread {
                    audioList.clear()
                    val document = Jsoup.connect("http://www.tingbook.cc/book/1-$page.html").get()
        ...
        binding.page.onLoadMore {
            runCatching {
                Thread {
                    ...
                    loge(stringBuilder.toString())
                    sendEvent(audioList, "load_more_models_xh")
                    page++
                    this.finish(true)
                }.start()
            }.onFailure {
