...
import com.manchuan.tools.activity.FullscreenGameActivity
import com.manchuan.tools.databinding.FragmentGameGbaBinding
...
class GameGBAFragment : EngineFragment<FragmentGameGbaBinding>(R.layout.fragment_game_gba) {
    ...
    override fun initView() {
        ...
        binding.recyclerView.grid(2, HoverGridLayoutManager.VERTICAL).setup {
            ...
            onClick(R.id.card) {
                context.startActivity<FullscreenGameActivity>(Intent.FLAG_ACTIVITY_CLEAR_TOP,
                    bundle = arrayOf(
                        "url" to getModel<GameModel>(modelPosition).playUrl
                    ))
            }
        }
        receiveEvent<ArrayList<GameModel>>("models_gba") {
            binding.recyclerView.models = it
        }
        receiveEvent<ArrayList<GameModel>>("load_more_models_gba") {
        ...
        binding.page.onRefresh {
            ...
            Thread {
                runCatching {
                    gameList.clear()
                    val document =
                        Jsoup.connect("https://www.yikm.net/nes?page=$page&tag=&e=2").get()
        ...
        binding.page.onLoadMore {
            Thread {
                runCatching {
                    ...
                    sendEvent(gameList, "load_more_models_gba")
                    page++
                    this.finish(true)
                }.onFailure {
                    this.finish(false)
