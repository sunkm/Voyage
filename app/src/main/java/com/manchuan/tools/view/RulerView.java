...
import static com.lxj.androidktx.AndroidKTX.context;
...
import com.manchuan.tools.extensions.ColorExtensionsKt;
import com.manchuan.tools.utils.ColorUtils;

public class RulerView extends android.view.SurfaceView implements android.view.SurfaceHolder.Callback {
    ...
    private void init(Context context) {
        ...
        paint = new Paint();
        paint.setColor(ColorExtensionsKt.accentColor(context));
        linePaint = new Paint();
        linePaint.setColor(ColorExtensionsKt.accentColor(context));
        linePaint.setStrokeWidth(4);
        fontPaint = new Paint();
        fontPaint.setTextSize(FONT_SIZE);
        fontPaint.setAntiAlias(true);
        fontPaint.setColor(ColorExtensionsKt.accentColor(context));
        lineX = PADDING;
        kedu = 0;
    }
    ...
    private void drawDisplay(Canvas canvas) {
        ...
        displayPaint1.setColor(ColorExtensionsKt.accentColor(context));
        ...
        displayPaint2.setColor(ColorExtensionsKt.windowBackground(context));
        ...
        canvas.drawCircle(SCREEN_W >> 1, SCREEN_H >> 1, RADIUS_BIG, cyclePaint);
        canvas.drawCircle(SCREEN_W >> 1, SCREEN_H >> 1, RADIUS_MEDIUM, cyclePaint);
        canvas.drawCircle(SCREEN_W >> 1, SCREEN_H >> 1, RADIUS_BIG, strokPaint);
        strokPaint.setColor(0xff666666);
        canvas.drawCircle(SCREEN_W >> 1, SCREEN_H >> 1, RADIUS_MEDIUM, strokPaint);
        strokPaint.setColor(0xff999999);
        canvas.drawCircle((SCREEN_W >> 1) + RADIUS_BIG, SCREEN_H >> 1,
                RADIUS_SMALL, cyclePaint);
        canvas.drawCircle((SCREEN_W >> 1) + RADIUS_BIG, SCREEN_H >> 1,
                RADIUS_SMALL, strokPaint);
        canvas.drawText(cm, (SCREEN_W >> 1) - cmWidth / 2,
                (SCREEN_H >> 1) + (bounds1.height() >> 1), displayPaint1);
        canvas.drawText(mm, (SCREEN_W >> 1) + RADIUS_BIG - mmWidth / 2, (SCREEN_H >> 1) + (bounds2.height() >> 1), displayPaint2);
    }
    ...
    private void draw() {
        ...
        try {
            ...
            canvas.drawColor(ColorUtils.getStatusBarColor());
            float left = PADDING;
